...
async function init() {
  ...
  const workspaceTriggerParent = document.getElementById(
    "workspace-trigger-parent",
  );
  const workspaceTriggerName = document.getElementById(
  ...
  const tileManager = createTileManager({
    ...
    onTerminalTileClosed(sessionId) {
      terminalListWebview.send("terminal-list:remove", sessionId);
    },
    onTileFocused(tile) {
  ...
  function resolveSurface(surface = lastNonModalSurface) {
    ...
    if (
      surface === "nav" &&
      !(panelManager.isVisible() && workspaceManager.getActiveWorkspace())
    ) {
      surface = null;
    }
    if (surface === "viewer") return "viewer";
    if (surface === "nav") return "nav";
    if (panelManager.isVisible() && workspaceManager.getActiveWorkspace())
      return "nav";
  ...
  terminalListWebview.webview.addEventListener("dom-ready", async () => {
    const discovered = (await window.shellApi.ptyDiscover?.()) ?? [];
    const initEntries = [];
    for (const [id] of tileManager.getTileDOMs()) {
      const tile = getTile(id);
      if (tile?.type === "term" && tile.ptySessionId) {
        const disc = discovered.find((d) => d.sessionId === tile.ptySessionId);
        initEntries.push({
          sessionId: tile.ptySessionId,
    ...
    terminalListWebview.send("terminal-list:init", initEntries);
  ...
  terminalListWebview.webview.addEventListener("ipc-message", (event) => {
